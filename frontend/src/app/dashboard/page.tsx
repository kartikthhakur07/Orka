  ...
  return (
    <div style={{ position: 'relative', display: 'inline-flex', alignItems: 'center', justifyContent: 'center' }}>
      <svg width={72} height={72}>
        <circle cx={36} cy={36} r={r} fill="none" stroke="rgba(203,213,225,0.4)" strokeWidth={5} />
        <circle
...
function AssignmentRow({ a, idx }: { a: any; idx: number }) {
  ...
  const colors = ['#ea580c', '#7c3aed', '#0d9488', '#16a34a', '#d97706', '#dc2626']
    ...
    <div className="offline-banner flex items-center gap-3">
      ...
      <div>
        ...
        <p style={{ fontSize: '0.8rem', opacity: 0.85 }}>
          If backend is sleeping, it takes ~30s to wake up. Retrying automatically...
        </p>
      </div>
    </div>
  )
...
export default function DashboardPage() {
  ...
  const stats = data ? [
    ...
    { label: 'WFH Rate',       value: data.wfh_rate       ?? data.wfhRate       ?? 0, icon: Home,          delay: 240, color: '#7c3aed' },
      ...
      {!loading && !error && projects.length === 0 && assignments.length === 0 && (
        <div className="glass-card p-10 text-center">
          ...
          <p style={{ color: 'var(--text-secondary)', fontWeight: 500 }}>
            Dashboard loaded — connecting to live backend endpoints!
          </p>
        </div>
      )}
    </div>
